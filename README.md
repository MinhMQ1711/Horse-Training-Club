# EquiFlow FE — Frontend hệ thống quản lý CLB đua ngựa
...
6. [`src/` — mã nguồn](#6-src--mã-nguồn)
   ...
   - [6.6 `types/`](#66-srctypes--kiểu-dữ-liệu-dùng-chung)
   - [6.7 `styles/`](#67-srcstyles--giao-diện-toàn-cục)
...
**Về file `.gitkeep`:** Git không lưu thư mục rỗng. Vì vậy các thư mục chưa có code đều chứa một file `.gitkeep` rỗng để Git giữ được thư mục. Khi bạn thêm file thật vào thư mục, có thể xóa `.gitkeep` đi (không bắt buộc).
...
| `Sidebar` | Menu điều hướng bên trái. **Đọc danh sách mục menu từ `lib/permissions.ts`** nên mỗi vai trò chỉ thấy mục của mình |
| `Topbar` | Thanh trên: thông tin người dùng, đăng xuất, thông báo |
| `RoleGuard` | Bọc route: nếu vai trò không đủ quyền thì chuyển tới `Forbidden403`; nếu chưa đăng nhập hoặc hết phiên thì tới `SessionExpired`/Login. Cũng đọc từ `lib/permissions.ts` |

#### `components/ui/` — thành phần giao diện

| Thành phần | Chức năng |
|---|---|
| `Button` | Nút bấm (các biến thể: chính, phụ, nguy hiểm...) |
| `StatusBadge` | Nhãn màu hiển thị trạng thái (ACTIVE, LOCKED...). Nhãn và màu lấy từ `lib/status.ts` |
...
#### `components/form/` — thành phần form
...
- Có **ít nhất 1 tài khoản cho mỗi trạng thái** (7 trạng thái).
- Mỗi trang mới phải có dữ liệu mock tương ứng.

### 6.6 `src/types/` — kiểu dữ liệu dùng chung
...
### 6.7 `src/styles/` — giao diện toàn cục
...
- `components/`: chỉ chứa thành phần riêng của feature. Nếu thấy dùng được ở feature khác thì chuyển lên `src/components/`.
- `api.ts`: ví dụ `listHorses()`, `getHorse(id)`. Bên trong gọi `lib/api.ts`, **không gọi `fetch` trực tiếp**.
- `types.ts`: hiện đang rỗng ở cả 10 feature.
...
| Người | Phụ trách |
|---|---|
| **FE1** | `components/` dùng chung, `auth`, `accounts`, `horses`, `intake`, `master-data` (P1–P2) |
...
> Thứ tự gợi ý: viết `styles/` (tokens, fonts, global) và `components/` dùng chung **trước**, vì mọi feature đều phụ thuộc vào chúng.
...
| Thêm loại ô nhập liệu mới | `components/form/` |
| Gọi một API mới | Thêm hàm vào `features/<nghiệp vụ>/api.ts` (dùng `lib/api.ts`) |
| Thêm dữ liệu giả cho trang mới | `mock/` |
| Đổi nhãn/màu của một trạng thái | `lib/status.ts` |
| Đổi câu báo lỗi hiển thị | `lib/messages.ts` |
| Thêm kiểu dữ liệu dùng chung | `types/` (chỉ 1 feature dùng thì `features/<tên>/types.ts`) |
| Đổi màu chủ đạo, cỡ chữ, bo góc | `styles/tokens.css` |
| Thêm font | Đặt file vào `public/fonts/`, khai báo trong `styles/fonts.css` |
