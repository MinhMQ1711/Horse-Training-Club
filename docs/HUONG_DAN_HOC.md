...
LoginPage (features/auth/pages)         ← giao diện + kiểm tra ô nhập
   │ gọi signIn(...)
   ▼
AuthProvider (shared/components/layout)        ← giữ "ai đang đăng nhập" cho cả app (React Context)
   │ gọi login(...)
   ▼
...
**D. Phân quyền RBAC — `shared/lib/permissions.ts`, `RoleGuard.tsx`, `Sidebar.tsx`**
...
- **Kiểm tra ở cả UI và API:** `shared/mock/handlers.ts` có `requirePermission` — client bị sửa tay thì server (mock) vẫn từ chối bằng 403.
...
**E. Cổng gọi API — `shared/lib/api.ts`**
- Một hàm `api(method, path, body)`. Mock bật → `import("@/shared/mock/handlers")` (nạp động, chỉ khi cần). Tắt → `fetch` thật kèm `credentials: "include"` (cookie phiên).
- Lỗi được gói thành `ApiError(status, code, ...)`. **401 + `UNAUTHENTICATED`** → mở Session Expired; **403 + `FORBIDDEN`** → trang 403.
- `shared/lib/messages.ts` đổi mã lỗi (`INVALID_CREDENTIALS`, `OTP_INVALID`...) thành câu tiếng Anh dễ hiểu.
...
- Màu, cỡ chữ, bo góc... chỉ dùng biến trong `shared/styles/tokens.css` (`var(--brand)`), **không** viết mã hex. Đổi giao diện = đổi token.
...
**H. Field + Context nối label/lỗi vào ô nhập** — `shared/components/form/Field.tsx`
...
**J. OTP (mã xác minh qua email)** — `OtpInput.tsx`, `OtpVerifyForm.tsx`, `shared/mock/handlers.ts`
...
- `useCountdown` (trong `shared/lib/hooks.ts`) đếm ngược thời hạn và thời gian chờ gửi lại.

**K. Mock backend** — `shared/mock/handlers.ts`, `shared/mock/db.ts`, `shared/mock/accounts.ts`
...
| Muốn... | Mở |
|---|---|
| Đổi màu, cỡ chữ, bo góc | `src/shared/styles/tokens.css` |
| Đổi vai trò nào thấy menu nào / được quyền gì | `src/shared/lib/permissions.ts` |
| Đổi câu báo lỗi | `src/shared/lib/messages.ts` |
| Đổi nhãn/màu của một trạng thái tài khoản | `src/shared/lib/status.ts` |
| Đổi hạn OTP, số lần sai, thời gian chờ gửi lại | hằng số đầu `src/shared/mock/handlers.ts` (backend thật sẽ do server quyết định) |
| Đổi thời gian hết phiên (30 phút) | `IDLE_MS` trong `src/shared/components/layout/AuthProvider.tsx` |
| Thêm tài khoản mẫu | `src/shared/mock/accounts.ts` |
